...
import os
from math import *
"""Implements the new GAFF hydoxyl parameterization of Fennell, Wymer, and Mobley (2014), which
involves scaling partial charges on hydroxyl and some surrounding atoms, and new LJ parameters
for hydroxyl oxygens.
Written by David Mobley, modeled after hydroxynator.pl by Chris Fennell. This means the current
implementation is relatively un-Pythonic, but it works.
Change Log:
    - First version, 11/20/13
    - 11/21/13: Fixed bug with tolerance for hydrogen bond lengths; fixed output file name; added
    option for output topology file name to have different name from input topology file.
"""
...
"Note: Assumes hydroxyl oxygens and hydrogens follow standard GAFF naming ('%s' and '%s' respectively; if you have hydroxyls with other atom names you will need to adjust the source code.)." % (hydroxyl_o, hydroxyl_h))
...
for line in text:
    ...
    if len(tmp)>1 and tmp[0][0] <> ';' and tmp[0][0] <> '[': 
        if types_sec and sigmaScale <> 1. and epsilonScale <> 1.: #If we're scaling LJ and we're in the atom types section
            ...
            bond_j.append( tmp[1] )
            if len(tmp) > 3:
                print("checking bond length")
                bond_lengths.append( float(tmp[3]) )
            else:
                raise Exception('No bond length found. Aborting.')
...
#Now identify hydroxyl bonded carbon atoms. I.e. find heavy atoms bonded to Hydroxyl O
other_bonded = [] 
...
#Loop over the other bondeds to identify hydrogens. I.e. find atoms bonded to heavy atoms bonded to Hydroxyl O -- Hydrogesn are scaled, while heavy atoms are neutralizing sites.
for i in range(len(other_bonded)):
...
#Scan for conflicts in the full_scale and neutralize_atoms lists. I.e. atoms within the neighborhood of two hydroxyls
dont_neutralize = []
...
#Get initial excess charge
initial_excess = 0.;
for i in range(len(atom_columns[6])):
    initial_excess += atom_columns[6][i] 
print(".top started with {} excess charge".format(initial_excess))
...
if charge_diff != 0:
    print("Excess charge {} after scaling, now redistributing".format(charge_diff))
if len(neutralize_atoms)>0:
    print("{} atoms to neutralize with".format(len(neutralize_atoms)))
    ...
    print("No neutralization atoms, subtracting equally from all")
    charge_diff /= float(len(atom_columns[6]))
    for i in range(len(atom_columns[6])):
        atom_columns[6][i] -= charge_diff 

#Now distribute the initial excess charge over the modified atoms
print("Now re-distributing the initial excess charge over atoms whose charges were modified by this script")
initial_excess /= ( float(len(neutralize_atoms) + len(full_scale)) )
for ii in range(len(neutralize_atoms)):
    atom_columns[6][ii] -= initial_excess
for ii in range(len(full_scale)):
    atom_columns[6][ii] -= initial_excess
...
for idx in range(atomstart, atomend):
    ...
    if len(tmp)>1:
        if not ';' in tmp[0] and not '[' in tmp[0]: 
            qtot += atom_columns[6][outct]
            print("qtot thus far: {}".format(qtot))
            mass = atom_columns[7][outct].split(";")[0]
            outtext[idx] = "%6d%11s%7d%8s%6s%7d%11.7f%11.6f ; %11.7f\n" % ( int(atom_columns[0][outct]),
                atom_columns[1][outct], int(atom_columns[2][outct]), atom_columns[3][outct], 
                atom_columns[4][outct], int(atom_columns[5][outct]), round(atom_columns[6][outct],7), 
                float(mass), qtot ) 
            outct+=1
