...
for adir in alldirs:
  ...
  print("Reading {} and {}".format(parmfile,rstfile))
  thisstruc = pmd.load_file(parmfile, xyz=rstfile)
  """
  print(adir)
  
  molname = adir.split("/")[0]
  parmfile = './'+adir+molname+'_scaled.top'
  #grofile = './'+adir+molname+'.gro'
  print("Reading {}".format(parmfile))

  thisstruc = pmd.load_file(parmfile)
